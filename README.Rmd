...
In the single cell World, which includes flow cytometry, mass cytometry, single-cell RNA-seq (scRNA-seq), and others, there is a need to improve data visualisation and to bring analysis capabilities to researchers even from non-technical backgrounds. *scToolkit* [@scToolkit] attempts to fit into this space, while also catering for advanced users. Additonally, due to the way that *scToolkit* is designed, which is based on *SingleCellExperiment* [@Lun], it has a 'plug and play' feel, and immediately lends itself as flexibile and compatibile with studies that go beyond *scToolkit*. Finally, the graphics in *scToolkit* are generated via the *ggplot* [@Wickham] engine, which means that users can 'add on' features to these with ease.
...
```{r ex5, message = FALSE, fig.height = 8, fig.width = 16, fig.cap = "clusKNN"}
