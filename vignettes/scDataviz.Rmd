...
In a practical situation, we would normally read in the data from the raw FCS files and then QC, normalise, transform, and filter them. This can be achieved via the `processFCS` function, which, by default, removes variables based on low variance and also downsamples [randomly] your data to 100000 variables. The user can change these via the `downsample` and `downsampleVar` parameters. An example (*not run*) is given below:
...
```{r readFCS, eval = FALSE}
  ...
  save(sce, file = 'scDataviz/data/complosome.rdata')

```

For this vignette, to save time, we will work with a smaller pre-prepared dataset that has been downsampled to 10000 cells using the above code.
...
```{r ex1, fig.height = 7, fig.width = 8, fig.cap = "Perform principal component analysis"}
  ...
  biplot(p,
    ...
    caption = '10000 cells randomly selected after having filtered for low variance')
